...
# Boltz integration for cl-revenue-ops (auto-generated)
revenue-ops-boltz-enabled=true
revenue-ops-boltz-use-sudo=false
revenue-ops-boltz-datadir=/data/boltz
revenue-ops-boltz-btc-wallet=${REVENUE_OPS_BOLTZ_BTC_WALLET:-CLN}
revenue-ops-boltz-lbtc-wallet=${REVENUE_OPS_BOLTZ_LBTC_WALLET:-LOOP-LBTC}
revenue-ops-boltz-daily-budget-sats=${REVENUE_OPS_BOLTZ_DAILY_BUDGET_SATS:-3000}
EOF
fi
...
if [ "$BOLTZ_ENABLED" = "true" ]; then
    ...
    # Symlink for lightning user so plugin/runtime checks can use boltzcli defaults too
    mkdir -p /home/lightning
    ln -sf /data/boltz /home/lightning/.boltz
...
BEOF
    ...
    # Ensure lightning user can access Boltz data/RPC auth generated by boltzd
    chown -R lightning:lightning /data/boltz /home/lightning/.boltz 2>/dev/null || true
    chmod 700 /data/boltz 2>/dev/null || true
    echo "Boltz client: enabled (datadir=/data/boltz)"
    echo "Boltz client: cl-revenue-ops integration enabled (wallets: BTC=${REVENUE_OPS_BOLTZ_BTC_WALLET:-CLN}, LBTC=${REVENUE_OPS_BOLTZ_LBTC_WALLET:-LOOP-LBTC})"
else
